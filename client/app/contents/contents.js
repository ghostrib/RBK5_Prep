angular
  ...
  .controller('ContentsController', function(
    ...
    Contents.getLectureByTitle($scope.titleURL).then(function(resp) {
      ...
      Contents.getLectureQuestions(resp.lecture._id).then(function(resp) {
        ...
        for (var i = 0; i < $scope.questions.length; i++) {
          ...
          if ($scope.questions[i]['example']) {
            if ($scope.questions[i]['example'].indexOf(';;') > -1) {
              $scope.questions[i]['example'] = $scope.questions[i][
                'example'
              ].replace(/;;/g, '\r\n');
            }
            if ($scope.questions[i]['example'].indexOf('\t') > -1) {
          ...
          if ($scope.questions[i]['postQuestion']) {
            if ($scope.questions[i]['postQuestion'].indexOf('\n') > -1) {
              $scope.questions[i]['postQuestion'] = $scope.questions[i][
                'postQuestion'
              ].replace(/\n/g, '\r\n');
            }
